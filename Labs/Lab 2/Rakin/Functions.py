...
def variance(inpic):
    ...
    [x, y] = np.meshgrid(
        np.linspace(-(usize - 1) / 2, (usize - 1) / 2, int(usize)),
...
def contour(img):
    ...
    mask = (
        (img * imgu <= 0) & (np.abs(img) < np.abs(imgu))
        | (img * imgl <= 0) & (np.abs(img) < np.abs(imgl))
        | (img * imgr <= 0) & (np.abs(img) < np.abs(imgr))
        | (img * imgd <= 0) & (np.abs(img) < np.abs(imgd))
    ).astype(int)
    return mask
...
# The format of these curves is a tuple of two arrays with (Y,X) coordinates.
#
def overlaycurves_EDITED(img, curves, checker=True):
    [h, w] = np.shape(img)
    rgb = np.zeros((h, w, 3), dtype=np.uint8)
    rgb[:, :, 0] = rgb[:, :, 1] = rgb[:, :, 2] = img
    (Y, X) = curves
