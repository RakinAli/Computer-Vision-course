...
def cmf_cut(ur, alpha):
    ...
    for i in range(numIter):
        ...
        pts = divp - (ps - pt + u / cc)
        pp1[:, 1:cols] = pp1[:, 1:cols] + steps * (
            pts[:, 1:cols] - pts[:, 0 : cols - 1]
        )
        pp2[1:rows, :] = pp2[1:rows, :] + steps * (
            pts[1:rows, :] - pts[0 : rows - 1, :]
        )
        ...
        gk = (
            np.where(gk <= alpha, 1, 0) + np.where(gk <= alpha, 0, 1) * (gk / alpha)
        ).astype(np.float32)
        gk = 1 / gk
...
def graphcut_segm(I, area, K, alpha, sigma):
    ...
    gauss = np.array(
        [[math.exp(-(i * i) / (2 * blur * blur)) for i in range(-3, 4)]],
        dtype=np.float32,
    )
...
def graphcut_example():
    ...
    sigma = 30.0  # edge cost decay factor
